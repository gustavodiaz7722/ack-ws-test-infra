...
#
# Changing PROW_VERSION or TOOLS_VERSION resets PROW_PATCH_REVISION to 1, since
# the new upstream tag has not been patched yet. Use --bump-patch to rebuild the
# current Prow version against newly published OS security updates without
...
if [[ "$CRD_ONLY" == "false" ]]; then
  ...
  if [[ "$VERSION_CHANGED" == "true" ]]; then
    # New upstream tag: no patched build exists for it yet.
    TARGET_REV=1
